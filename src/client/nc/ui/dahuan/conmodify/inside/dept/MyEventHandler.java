...
public class MyEventHandler extends ManageEventHandler {
	...
	protected void onBoQuery() throws Exception {
		...
		String qxWhere = " and ( zdr='"+this._getOperator()+"' or shr='"+this._getOperator()+"' )";
	...
	protected void onBoSave() throws Exception {
		...
		String sqlhttype="select httype from dh_contract t where t.pk_contract='"+patconid+"'";
		Integer it=(Integer)iQ.executeQuery(sqlhttype, new ColumnProcessor());
		if(it==1){//采购合同
	...
	public void onBoAudit() throws Exception {
		...
		BudgetDialog rcmDg = new BudgetDialog(this.getBillUI());
		boolean flag = rcmDg.showBudgetDialog();
		if(flag){
			mvo.setBudget(rcmDg.getBudget());
			mvo.setModify_status(2);
			mvo.setShdate(this._getDate());
			HYPubBO_Client.update(mvo);
			this.onBoRefresh();
		}
	}
	...
	protected void onBoCancelAudit() throws Exception {
		...
		ConModfiyVO mvo = (ConModfiyVO)aggvo.getParentVO();
		mvo.setBudget(mvo.getOld_budget());
		mvo.setModify_status(1);
		mvo.setShdate(this._getDate());
		HYPubBO_Client.update(mvo);
		this.onBoRefresh();
