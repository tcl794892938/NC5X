...
public class RefContractModel extends AbstractRefModel {
	...
	public RefContractModel() {
		...
		setRefNodeName("合同");
		setRefTitle("合同");
		setFieldCode(new String[] {"t.ctcode","(select f.ctcode from dh_contract f where t.relationid = f.pk_contract) fctcode",
		...
		setFieldName(new String[] {"合同编号","关联合同号","合同名称","项目名称","合同金额"});
		setHiddenFieldCode(new String[] { "t.pk_contract","t.relationid" });
	...
	private String getConfString(){
		...
		if(pkCorp.equals("1002")){//过滤溧阳
			return " and t.vapproveid = '"+pkUser+"' and t.pk_corp = '"+pkCorp+"' and nvl(t.is_delivery,0)=0 " +
			"and t.is_relation = 1 and nvl(t.dr,0)=0 and t.is_seal = 1 " +
			"and not exists (select 1 from bd_jobmngfil m where m.pk_jobmngfil=t.pk_jobmandoc and nvl(m.dr,0)=0 and m.sealflag='Y')";//by tcl
		}
		...
		if(it>0){//部门主任(销售合同权限)
			return " and t.vapproveid = '"+pkUser+"' and t.pk_corp = '"+pkCorp+"' and nvl(t.is_delivery,0)=0 " +
			"and t.is_relation = 1 and httype=0 and nvl(t.dr,0)=0 and t.is_seal = 1 " +
			"and not exists (select 1 from bd_jobmngfil m where m.pk_jobmngfil=t.pk_jobmandoc and nvl(m.dr,0)=0 and m.sealflag='Y')";//by tcl
		}
		...
		if(it2>0){
			return " and t.pk_corp = '"+pkCorp+"' and nvl(t.is_delivery,0)=0 " +
			"and t.is_relation = 1 and httype=1 and nvl(t.dr,0)=0 and t.is_seal = 1 " +
			"and not exists (select 1 from bd_jobmngfil m where m.pk_jobmngfil=t.pk_jobmandoc and nvl(m.dr,0)=0 and m.sealflag='Y')";//by tcl
		}
