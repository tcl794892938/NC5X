...
//		getBillListPanel().getParentListPanel().setTotalRowShow(true);//表头
		...
		// 制单
		ButtonObject edit = (ButtonObject)this.getButtonManager().getButton(IdhButton.FKZD);
	...
	public void afterEdit(BillEditEvent e) {
		...
		if(e.getKey().equalsIgnoreCase("dfkbl")){   
		    ...
		    String formula2 = "currenty_amount->dfkje/currenty_rate";
		    this.getBillCardPanel().execHeadFormula(formula2);
		}else if(e.getKey().equalsIgnoreCase("currenty_amount")){
			
			String formula = "dfkje->currenty_amount*currenty_rate";
			this.getBillCardPanel().execHeadFormula(formula);
			
			String formula2 = "dfkbl->dfkje*100/dctjetotal";
			this.getBillCardPanel().execHeadFormula(formula2);
		}
