...
import nc.ui.dahuan.hkjh.ClientUICheckRuleGetter;
 ...
 */
  public abstract class AbstractClientUI extends nc.ui.trade.manage.BillManageUI implements  ILinkQuery{
	...
	protected BusinessDelegator createBusinessDelegator() {
		return new nc.ui.dahuan.hkjh.yw.MyDelegator();
	}
	...
	protected void initPrivateButton() {
		...
		for (int i = 0; i < listButns.length; i++) {
			...
			if( listButns[i] == nc.ui.trade.button.IBillButton.CancelAudit )
				hasCancelAudit = true;
			if( listButns[i] == nc.ui.trade.button.IBillButton.Refbill )
				hasRefBill = true;
		}
		...
		for (int i = 0; i < cardButns.length; i++) {
			...
			if( cardButns[i] == nc.ui.trade.button.IBillButton.CancelAudit )
				hasCancelAudit = true;
			if( cardButns[i] == nc.ui.trade.button.IBillButton.Refbill )
				hasRefBill = true;
		}		
		if( hasCommit ){
		...
		if( hasRefBill ){
			ButtonVO btnVo4 = nc.ui.trade.button.ButtonVOFactory.getInstance()
			.build(nc.ui.trade.button.IBillButton.Refbill);
			btnVo4.setBtnCode(null);
			btnVo4.setBtnName("确定");
