...
public class HTZXLYReportUI extends ReportUIBase {
	...
	protected void onQuery() throws Exception {
		...
		if (getQryDlg().showModal() == QueryDLG.ID_OK) {
			...
			if(lastCustomQueryCondition.indexOf("zdy.yll")!=-1){//条件包含盈利率
			...
			if(lastCustomQueryCondition.indexOf("zdy.fhbz")!=-1){ //条件包含发货标识
	...
	protected void setPrivateButtons() {
		...
		ButtonObject m_boQueryfpmx = new ButtonObject("发票联查明细", "发票联查明细", 0,
				"发票联查明细");
		//ButtonObject m_boQueryll = new ButtonObject("利率", "利率", 0,"利率");
	...
	private ArrayList doFilterData(String zdy_fhbs,String zdy_yll,ArrayList list) throws Exception{
		...
		if(!"".endsWith(zdy_fhbs)){
			...
			if(it==0){ //未发货
				for(int i=0;i<list.size();i++){
					HtzxReportVO  vo=(HtzxReportVO)list.get(i);
					String str= vo.getIffh()==null?"已发货":vo.getIffh();
					if(!str.equals("未发货")){
						list.remove(i);
						i--;
					}
				...
				for(int i=0;i<list.size();i++){
					HtzxReportVO  vo=(HtzxReportVO)list.get(i);
					String str= vo.getIffh()==null?"未发货":vo.getIffh();
					if(!str.equals("已发货")){
						list.remove(i);
						i--;
					}
