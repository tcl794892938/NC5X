...
public class HTZXReportUI extends ReportUIBase {
	...
	protected void onQuery() throws Exception {
		...
		if (getQryDlg().showModal() == QueryDLG.ID_OK) {
			...
			if (node.equalsIgnoreCase("12H20301")) {
				...
				if (num <= 0) {
					this.showErrorMessage("请选择部门信息查询!");
					return;
				}
			}
			...
			for (int i = 0; i < condvos.length; i++) {
				if (condvos[i].getFieldCode().equalsIgnoreCase(
					...
					if (pk_dept.equalsIgnoreCase("自动化部")) {
						lastCustomQueryCondition = lastCustomQueryCondition
								.replaceAll("TEMQ_CT.pk_deptdoc1",
										"TEMQ_CT.pk_deptdoc");
						
						lastCustomQueryCondition =lastCustomQueryCondition
						.replaceAll("TEMQ_CT.pk_deptdoc = '自动化部'", "TEMQ_CT.pk_deptdoc = '自动化部' or TEMQ_CT.jobname like '%电气%'");
					}
			...
			if (list.size() != 0) {
				...
				for (int i = 0; i < list.size(); i++) {
					...
					if (expensehtzxVo.getJobname().endsWith("-成本")) {
						expensehtzxVo.setCgj(expensehtzxVo.getSjfy());
						expensehtzxVo.setCe(expensehtzxVo.getSjfy()
								.multiply(-1));
					}
					...
					if (1 == isdel) {
						expensehtzxVo.setIffh("已发货");
					} else {
						expensehtzxVo.setIffh("未发货");
					}
				...
				for (String mapcode : pxlit) {
					...
					if (val.size() > 1) {
						...
						for (int i = 0; i < val.size(); i++) {
							...
							if (!ctname.equalsIgnoreCase("零星采购")
									&& !itemvotmp.getJobname().endsWith("-费用")
									&& !itemvotmp.getJobname().endsWith("-成本")) {
				...
				// //设置报表表体不可排序
				// ((BillCardPanel)getReportBase()).getBillTable().removeSortListener();//设置报表表体不可排序
	...
	protected void onClearItemyj() throws Exception {
		...
		if (itemvos == null || itemvos.length <= 0) {
			this.showErrorMessage("表体数据为空，不可以过滤！");
			return;
		}
	...
	protected void onQueryYcht() throws Exception {
		...
		if (itemvos == null || itemvos.length <= 0) {
			this.showErrorMessage("表体数据为空，不可以显示！");
			return;
		}
	...
	protected void setPrivateButtons() {
		...
		ButtonObject m_boQuerymx = new ButtonObject("收付联查明细", "收付联查明细", 0,
				"收付联查明细");
		ButtonObject m_boQueryfpmx = new ButtonObject("发票联查明细", "发票联查明细", 0,
				"发票联查明细");
		ButtonObject m_boqcyjmx = new ButtonObject("过滤已结", "过滤已结", 0, "过滤已结");
		ButtonObject m_boycht = new ButtonObject("异常合同", "异常合同", 0, "异常合同");

		ButtonObject m_boyfh = new ButtonObject("领料明细", "领料明细", 0, "领料明细");
		ButtonObject m_zbxj = new ButtonObject("总包分类小计", "总包分类小计", 0, "总包分类小计");
		ButtonObject m_ycpz = new ButtonObject("异常凭证", "异常凭证", 0, "异常凭证");
	...
	protected void onQueryMx() throws Exception {
		...
		if (selectvo == null) {
			this.showErrorMessage("请先选择要查看明细的数据!");
			return;
		}
		...
		String QueryMxSql = "select TEMQ_CT.jobname as jobname,                                                      "
				...
				+ "       gl_voucher.period <> '00' and gl_detail.explanation <> '期初')                   "
	...
	protected void onQueryFh() throws Exception {
		...
		// /关闭已经打开的窗体 ,以便后续重新打开进行加装数据 start
	...
	protected void onQueryZb() throws Exception {
		...
		// 画面增加总包小计(包含：机械，电气，土建安装，土建设计，设计) by tcl
		...
		int g = 0;// 计数(表示当前的行数)
		String str = "";// 记录合同类型
		ArrayList<Integer> listint = new ArrayList<Integer>();// 计数那些行是小计
		LinkedHashMap<String, HtzxReportVO> map = new LinkedHashMap<String, HtzxReportVO>();
		listitem.add(itemvos[0]);// 合计行不变

		boolean flag = false;// 进入和出来标志
		String mapkey = null;// 记录map当前的key
		for (int i = 1; i < itemvos.length; i++) {
			HtzxReportVO vo = itemvos[i];

			if (vo.getJobname().indexOf("-机械") > 0) {
				str = "-机械";
			} else if (vo.getJobname().indexOf("-电气") > 0) {
				str = "-电气";
			} else if (vo.getJobname().indexOf("-土建安装") > 0) {
				str = "-土建安装";
			} else if (vo.getJobname().indexOf("-土建设计") > 0) {
				str = "-土建设计";
			} else if (vo.getJobname().indexOf("-设计") > 0) {
				str = "-设计";
			} else {
				flag = false;
				if (mapkey != null) {
					...
					listitem.add(newvo);
					g++;
					listint.add(g);// 加入listint
					mapkey = null;// 清空mapkey
				}
				listitem.add(vo);// 先将数据加入到listitem
				g++;
				continue;
			}

			flag = true;
			String htzname = vo.getHtzname();// 合同组名称
			String key = htzname + str;
			if (mapkey != null && !mapkey.equals(key)) {// 跨合同类型
				HtzxReportVO newvo = map.get(mapkey);
				listitem.add(newvo);
				g++;
				listint.add(g);// 加入listint
	...
	protected void onQueryYCPZ() throws Exception {
		...
		if (!"".equals(str)) {
			MessageDialog.showHintDlg(getReportBase(), "提示", "合同编号为" + str
					+ "重复，请检查！");
		} else {
			MessageDialog.showHintDlg(getReportBase(), "提示", "无异常合同！");
		}
		return;
	}
