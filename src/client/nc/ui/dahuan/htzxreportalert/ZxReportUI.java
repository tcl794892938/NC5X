...
public class ZxReportUI extends ReportUIBase {
	...
	public ZxReportUI(FramePanel fp) {
		super(fp);
	}
	...
	protected void onQuery() throws Exception {
		...
		if (getQryDlg().showModal() == QueryDLG.ID_OK) {
            ...
            String querySql = "select bd_corp.unitname,dh_htalertbase.JOBCODE,dh_htalertbase.JOBNAME,dh_htalertbase.DEF10, dh_htalertbase.CUSTNAME,	dh_htalertbase.DEPTNAME,dh_htalertbase.xsj,"+
                  " isnull(dh_htalertbase.CGJ, 0.00)+isnull(dh_htalertbase.SJFY, 0.00) cgj,"+
                  " isnull(dh_htalertbase.XSJ, 0.00)-isnull(dh_htalertbase.CGJ, 0.00)-isnull(dh_htalertbase.SJFY, 0.00) ce "+
                  " from dh_htalertbase,bd_corp where dh_htalertbase.pk_corp = bd_corp.pk_corp "+
                  " and (dh_htalertbase.ALERTYPE = 1) and (isnull(dh_htalertbase.XSJ, 0.00)-isnull(dh_htalertbase.CGJ, 0.00)-isnull(dh_htalertbase.SJFY, 0.00) < 0) "+strCondition;
            ...
            for (int i = 0; i < list.size(); i++) {
				...
				if ((dzvo.getXsj() != null) &&(dzvo.getXsj().doubleValue() != 0)) {
				          UFDouble kslv = dzvo.getCe().div(dzvo.getXsj())
				            .multiply(100);
				dzvo.setKslv(kslv);
				} else {
