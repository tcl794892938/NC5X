...
public class ClientUI extends BillManageUI {
	...
	protected void initSelfData() {
		((ButtonVO)this.getButtonManager().getButton(IBillButton.Audit).getData()).setExtendStatus(new int[]{4});
	...
	protected int getExtendStatus(AggregatedValueObject vo) {
		if(null != vo){
			...
			if(0==pcs||3==pcs||5==pcs||7==pcs){
				...
				String fzr=pcvo.getFzr();
				String cper=ClientEnvironment.getInstance().getUser().getPrimaryKey();
				if(fzr.equals(cper)){
					return 4;
				}else{
					return 5;
