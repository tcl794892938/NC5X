package nc.vo.dahuan.report;

import nc.vo.pub.SuperVO;
import nc.vo.pub.lang.UFDouble;

public class HtzxReportVO extends SuperVO {

	public String jobname;

	public String def1;

	public UFDouble xsj;

	public UFDouble cgj;

	public UFDouble ce;

	public String deptname;

	public String custname;
	
	public UFDouble sjsk;
	
	public UFDouble sjfk;
	
	public UFDouble sjfy;

	public UFDouble sjml;

	public UFDouble sjkp;

	public UFDouble sjsp;

	public String subjname;
	
	public UFDouble fs;
	
	public String explanation;
	
	public String prepareddate;
	
	public String no;
	
	public String period;
	
	public String htzname;
	
	public String iffh;
	
	public String jobcode;
	
	public String projectname;
	
	public String unitname;
	
	public UFDouble html;
	
	
	
	
	

	

	public UFDouble getHtml() {
		return html;
	}

	public void setHtml(UFDouble html) {
		this.html = html;
	}

	public String getUnitname() {
		return unitname;
	}

	public void setUnitname(String unitname) {
		this.unitname = unitname;
	}

	public String getProjectname() {
		return projectname;
	}

	public void setProjectname(String projectname) {
		this.projectname = projectname;
	}

	public String getJobcode() {
		return jobcode;
	}

	public void setJobcode(String jobcode) {
		this.jobcode = jobcode;
	}

	public String getIffh() {
		return iffh;
	}

	public void setIffh(String iffh) {
		this.iffh = iffh;
	}

	public String getHtzname() {
		return htzname;
	}

	public void setHtzname(String htzname) {
		this.htzname = htzname;
	}

	public String getExplanation() {
		return explanation;
	}

	public void setExplanation(String explanation) {
		this.explanation = explanation;
	}

	public UFDouble getFs() {
		return fs;
	}

	public void setFs(UFDouble fs) {
		this.fs = fs;
	}

	public String getNo() {
		return no;
	}

	public void setNo(String no) {
		this.no = no;
	}

	public String getPeriod() {
		return period;
	}

	public void setPeriod(String period) {
		this.period = period;
	}

	public String getPrepareddate() {
		return prepareddate;
	}

	public void setPrepareddate(String prepareddate) {
		this.prepareddate = prepareddate;
	}

	public UFDouble getCe() {
		return ce;
	}

	public void setCe(UFDouble ce) {
		this.ce = ce;
	}

	public UFDouble getCgj() {
		return cgj;
	}

	public void setCgj(UFDouble cgj) {
		this.cgj = cgj;
	}

	public String getCustname() {
		return custname;
	}

	public void setCustname(String custname) {
		this.custname = custname;
	}

	public String getDef1() {
		return def1;
	}

	public void setDef1(String def1) {
		this.def1 = def1;
	}

	public String getDeptname() {
		return deptname;
	}

	public void setDeptname(String deptname) {
		this.deptname = deptname;
	}

	public String getJobname() {
		return jobname;
	}

	public void setJobname(String jobname) {
		this.jobname = jobname;
	}

	public UFDouble getSjkp() {
		return sjkp;
	}

	public void setSjkp(UFDouble sjkp) {
		this.sjkp = sjkp;
	}

	public UFDouble getSjml() {
		return sjml;
	}

	public void setSjml(UFDouble sjml) {
		this.sjml = sjml;
	}

	public UFDouble getSjsp() {
		return sjsp;
	}

	public void setSjsp(UFDouble sjsp) {
		this.sjsp = sjsp;
	}

	public String getSubjname() {
		return subjname;
	}

	public void setSubjname(String subjname) {
		this.subjname = subjname;
	}

	public UFDouble getXsj() {
		return xsj;
	}

	public void setXsj(UFDouble xsj) {
		this.xsj = xsj;
	}

	@Override
	public String getPKFieldName() {
		// TODO Auto-generated method stub
		return null;
	}

	@Override
	public String getParentPKFieldName() {
		// TODO Auto-generated method stub
		return null;
	}

	public String getTableName() {
		// TODO Auto-generated method stub
		return null;
	}

	public UFDouble getSjfk() {
		return sjfk;
	}

	public void setSjfk(UFDouble sjfk) {
		this.sjfk = sjfk;
	}

	public UFDouble getSjfy() {
		return sjfy;
	}

	public void setSjfy(UFDouble sjfy) {
		this.sjfy = sjfy;
	}

	public UFDouble getSjsk() {
		return sjsk;
	}

	public void setSjsk(UFDouble sjsk) {
		this.sjsk = sjsk;
	}

}
